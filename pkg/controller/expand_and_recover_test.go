...
func TestExpandAndRecover(t *testing.T) {
	...
	var tests = []struct {
		...
		{
			name:                  "pvc.spec.size > pv.spec.size, resize_status=node_resize_inprogress",
		...
		{
			name:                  "pvc.spec.size = pv.spec.size, resize_status=no_resize_inprogress",
		...
		{
			name:                  "pvc.spec.size > pv.spec.size, resize_status=controller_resize_failed",
		...
		{
			name:                       "pvc.spec.size = pv.spec.size, resize_status=node_resize_failed, disable_controller_resize=true",
		...
		{
			name:                  "pvc.spec.size = pv.spec.size, resize_status=node_resize_pending",
		...
		{
			name:                  "pvc.spec.size > pv.spec.size, disable_node_resize=true, resize_status=no_resize_inprogress",
			pvc:                   getTestPVC("test-vol0", "2G", "1G", "", ""),
			pv:                    createPV(1, "claim01", defaultNS, "test-uid", &fsVolumeMode),
		...
		{
			name:                  "pv.spec.size >= pvc.spec.size, resize_status=node_resize_failed",
	...
	for i := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("resize failed with %v", err)
			}
			if test.expectResizeCall != resizeCalled {
				t.Fatalf("resize failed: expected resize called %t, got %t", test.expectResizeCall, resizeCalled)
			}
			if pvc.Status.AllocatedResourceStatuses[v1.ResourceStorage] != test.expectedResizeStatus {
				t.Fatalf("expected resize status to be %s, got %s", test.expectedResizeStatus, pvc.Status.AllocatedResourceStatuses[v1.ResourceStorage])
			}

			actualAllocatedSize := pvc.Status.AllocatedResources.Storage()

			if test.expectedAllocatedSize.Cmp(*actualAllocatedSize) != 0 {
				t.Fatalf("resize failed: expected allocated size %s, got %s", test.expectedAllocatedSize.String(), actualAllocatedSize.String())
			}
...
func getTestPVC(volumeName string, specSize, statusSize, allocatedSize string, resizeStatus v1.ClaimResourceStatus) *v1.PersistentVolumeClaim {
	...
	if len(resizeStatus) > 0 {
		pvc.Status.AllocatedResourceStatuses[v1.ResourceStorage] = resizeStatus
	}
	return pvc
}
